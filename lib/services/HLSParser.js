...
var HLSParser = /** @class */ (function () {
    ...
    HLSParser.findVariant = function (master, predicate) {
        var index = master.variants.findIndex(predicate);
        if (index < 0) {
            return undefined;
        }
        return {
    ...
    HLSParser.prototype.filterVariants = function (master, predicate) {
        return master.variants.filter(predicate).map(function (val, index) {
            return { variant: val, index: index };
        });
    };
    return HLSParser;
}());
