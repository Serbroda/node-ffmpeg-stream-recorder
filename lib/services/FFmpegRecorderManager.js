...
var FFmpegRecorderManager = /** @class */ (function () {
    ...
    Object.defineProperty(FFmpegRecorderManager.prototype, "options", {
        get: function () {
    ...
    FFmpegRecorderManager.prototype.create = function (request, onStateChange) {
        ...
        recorderOptions.onStateChange = function (newState, oldState, sessionInfo) {
            if (sessionInfo) {
                if (_this.recorders[sessionInfo.id]) {
                    _this.recorders[sessionInfo.id].request.state = newState;
                    if (onStateChange) {
                        onStateChange(_this.recorders[sessionInfo.id].request, newState);
                    }
                }
            }
        };
        var rec = new FFmpegRecorder_1.FFmpegRecorder(request.url, __assign(__assign({}, recorderOptions), {
